...
export default async function LtiToolkit(config) {
  ...
  if (config.provider && config.provider.key && config.provider.secret) {
    ...
    await config.database.queryInterface.bulkDelete("lti_consumer_keys", {}, { truncate: true });
    await config.database.queryInterface.bulkDelete("lti_consumers", {}, { truncate: true });

    // Create consumer and key
    await returnObj.controllers.consumer.createConsumer({
  ...
  if (config.provider) {
    ...
    returnObj.controllers.lti.provider = LTIProviderControllerInstance;
  }
...
function validateProviderConfig(providerConfig) {
  if (providerConfig && typeof providerConfig === "object") {
    ...
    if (providerConfig.key && typeof providerConfig.key !== "string") {
      throw new Error("provider.key must be a string");
    }
    if (providerConfig.secret && typeof providerConfig.secret !== "string") {
      throw new Error("provider.secret must be a string");
    }
  } else {
    providerConfig = null;
  }
