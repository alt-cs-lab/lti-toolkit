...
* `deployment_id` - deployment ID provided by the LTI Consumer
...
### lti_provider_deep_links

This table stores pending LTI 1.3 Deep Linking requests initiated by the Tool Consumer. The token is passed through the OIDC login flow so the Deep Link response can be matched back to the original request context. Entries in this table are automatically expired after 15-20 minutes (checked every 5 minutes; entries older than 15 minutes are removed).

* `token` - {{% badge %}}Auto{{% /badge %}} autogenerated one-time token identifying the pending deep link request. Generated using [nanoid](https://www.npmjs.com/package/nanoid)
* `provider_key` - {{% badge %}}LTI 1.3{{% /badge %}} key of the LTI Tool Provider that the deep link request was sent to
* `context` - {{% badge %}}Auto{{% /badge %}} JSON object containing context data stored when the deep link request was initiated (passed to the `handleDeeplink` callback when the response is received)
* `createdAt` - {{% badge %}}Auto{{% /badge %}} creation timestamp
* `updatedAt` - {{% badge %}}Auto{{% /badge %}} last updated timestamp