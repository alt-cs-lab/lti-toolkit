...
All notable changes to this project will be documented here. This project follows [Semantic Versioning](https://semver.org/) and the format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/).
...
- **At-rest encryption** — all stored secrets and RSA private keys are now encrypted using AES-256-GCM; requires a new `encryption_key` config option (64-character hex string)
...
- **TypeScript type declarations** — `index.d.ts` now includes typed interfaces for all four controllers (`ConsumerRegistryController`, `ProviderRegistryController`, `LTIConsumerController`, `LTIProviderController`) and their return shapes (`ConsumerInstance`, `ProviderInstance`, `ConsumerKeyData`, `ProviderKeyData`, `LaunchFormData`, etc.)
...
- **AGS token scopes** — the token endpoint now advertises all three scopes this library implements: `score`, `lineitem.readonly`, and `result.readonly` (previously only `score` was listed)
- **Dynamic Registration response** — the registration response is now filtered to the fields required by the LTI 1.3 spec, dropping extra fields that caused errors with some Tool Providers
- **OAuth 1.0 signature base URI** — the base URI now reads `X-Forwarded-Proto` and `X-Forwarded-Host` headers so signature verification works correctly behind a reverse proxy
- **Dynamic Registration URL validation** — the registration endpoint URL is now validated by comparing URL origins rather than a `startsWith` prefix check, preventing bypass with crafted URLs

### Removed

- **`provider.key` / `provider.secret` config shortcut** ⚠️ **Breaking** — this option previously created a single LTI 1.0 consumer at startup but also destroyed all other consumers on every restart. It has been removed. Use `consumerRegistry.createConsumer()` in application startup code to seed a default consumer instead.

### Fixed

- LTI 1.3 launches now validate `deployment_id` against the stored consumer record; mismatches are rejected with a clear error
- LTI 1.3 login state is no longer destroyed before `deployment_id` validation, preventing a race condition that could cause valid launches to fail
...
- All shared secrets and RSA private keys are now encrypted at rest using AES-256-GCM with a key derived from the new required `encryption_key` config option. Existing plaintext values in v1.0 databases must be re-entered after upgrading.
